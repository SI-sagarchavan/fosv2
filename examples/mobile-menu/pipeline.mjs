/**
 * The hybrid pipeline on a MOBILE frame — 375x667, so `inferBreakpoint` picks
 * the theme's mobile type set rather than desktop. A drawer, not a gallery, so
 * it has its own repairs: nav lists and hairline dividers.
 *
...
function stripPageSizing(props) {
  const next = { ...props };
  delete next.place;
  // Width yes, height NO. This page root is an Overlay — its children are
  // absolutely placed, so it has no intrinsic height and `auto` collapses the
  // whole page to the height of its header. A section that flows can hug its
  // content; a page that composes cannot.
  if (next.size && typeof next.size === "object") next.size = { ...next.size, w: "full" };
  return next;
}
...
step("render", () =>
  run("packages/renderer", [
    ...
    // A whole page, not a band: the capture viewport has to be tall enough or
    // the screenshot stops at the fold and everything below it goes unchecked.
    "--viewport", "700",
  ]).trim(),
);

step("diff vs reference-nav.png", () =>
  run("packages/renderer", [
    "report",
    "--tree", resolve(out, "2b-section.dsl.json"),
    "--data", DATA,
