/**
 * The hybrid pipeline on the Tickets hero.
 *
 * Same six stages as examples/news-section, a different set of gaps: a wordmark
 * that arrived as 14 vector paths, and a background that arrived as three
 * overlapping crops. See repairs.mjs.
 *
...
const IR = resolve(here, "tickets.ir.json");
const DATA = resolve(here, "data.json");
const THEME = resolve(repo, "packages/tokens/fixtures/southern-brave.json");
const REFERENCE = resolve(here, "reference-plate.png");

/** The section root itself — the hero IS the whole 1366x488 band. */
const ROOT_SRC = "1:4367";
...
/**
 * The IR is already rooted at the section, so re-rooting is a no-op here — it
 * runs anyway so both examples emit the same artifacts and the harness can read
 * them the same way.
 */
...
step("diff vs reference-plate.png", () =>
  run("packages/renderer", [
    "report",
    "--tree", resolve(out, "2b-section.dsl.json"),
    "--data", DATA,
